...
const EditReceiptRow = ({ receipt, onSave, onCancel }) => {
        ...
        <div className="space-y-4">
          {editedReceipt.sizes.map((size, index) => (
            <div key={index} className="p-3 bg-white rounded-lg shadow-sm">
              <label className="block mb-2 text-sm font-medium">Size {size.size}</label>
              <input
                type="number"
                pattern="[0-9]*"
                inputMode="numeric"
                ...
                placeholder="Pieces"
              />
              <div className="mt-1 text-sm text-gray-500">Total: {size.total}</div>
            </div>
          ))}
        </div>
  ...
  const DesktopView = () => (
    <>
      ...
      {editedReceipt.sizes.map((size, index) => (
        <td key={index} className="hidden p-2 align-middle md:table-cell md:p-4">
    ...
    <table className="w-full">
      <thead className="hidden md:table-header-group">
        <tr className="text-white bg-blue-500">
          <th className="h-12 px-4 font-medium text-left text-white align-middle">ID</th>
          <th className="h-12 px-4 font-medium text-left text-white align-middle">નામ</th>
          <th className="h-12 px-4 font-medium text-left text-white align-middle">સાઇડ</th>
          <th className="h-12 px-4 font-medium text-left text-white align-middle">મોબાઇલ</th>
          <th className="h-12 px-4 font-medium text-left text-white align-middle">Actions</th>
        </tr>
      </thead>
      <tbody>
        {userData.map((user) => (
          <React.Fragment key={user.userId}>
            ...
            >
              <td className="flex items-center justify-between p-4 md:table-cell">
              ...
              <td className="hidden p-4 md:table-cell">{user.name}</td>
              <td className="hidden p-4 md:table-cell">{user.site || 'N/A'}</td>
              <td className="hidden p-4 md:table-cell">{user.phone || 'N/A'}</td>
                    ...
                    <div className="p-4">
                        ...
                        <table className="w-full border border-collapse border-blue-200">
                          <thead>
                            <tr className="bg-blue-50">
                              <th className="p-2 text-center border border-blue-200" colSpan={returnItems[0]?.sizes?.length + 4}>
                                Size-wise Stock Status
                              </th>
                            </tr>
                            <tr className="bg-blue-50">
                              <th className="p-2 border border-blue-200">Type</th>
                              <th className="p-2 border border-blue-200">Udhar</th>
                              <th className="p-2 border border-blue-200">Jama</th>
                              <th className="p-2 border border-blue-200">Balance</th>
                              {returnItems[0]?.sizes?.map((size) => (
                                <th key={size.size} className="p-2 border border-blue-200">{size.size}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {(() => {
                              ...
                              return (
                                <tr>
                                  <td className="p-2 font-medium border border-blue-200">Current Stock</td>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="w-full min-w-full table-auto">
                          <thead>
                            <tr className="bg-blue-50">
                              <th className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">Receipt</th>
                              <th className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">Date</th>
                              <th className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">Total</th>
                              {returnItems[0]?.sizes?.map((size) => (
                                <th key={size.size} className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">
                                  {size.size}
                                </th>
                              ))}
                              <th className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">Notes</th>
                              <th className="h-10 px-2 text-sm font-medium text-left align-middle md:px-4">Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                            {returnItems.map((item) => (
                              <tr key={item.receiptNumber} 
                                ...
                                {editingReceipt?.receiptNumber === item.receiptNumber ? (
                                  ...
                                  <>
                                    <td className="p-2 text-sm font-medium text-blue-600 md:p-4">{item.receiptNumber}</td>
                                    <td className="p-2 text-sm md:p-4">{formatDate(item.date)}</td>
                                    <td className="p-2 text-sm md:p-4">{item.total}</td>
                                    {item.sizes?.map((size) => (
                                      <td key={size.size} className="p-2 text-sm text-center md:p-4">
                                        {size.total || '0'}
                                      </td>
                                    ))}
                                    <td className="p-2 text-sm md:p-4">{item.notes || 'N/A'}</td>
...
{showDeleteConfirm && (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
    <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
      <h3 className="mb-4 text-lg font-semibold text-blue-700">Confirm Delete</h3>
