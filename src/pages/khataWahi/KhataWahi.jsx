    ...
    <>
      ...
      <td className="p-2 md:p-4 align-middle">
        <input
          ...
          value={editedReceipt.date}
          onChange={(e) => setEditedReceipt(prev => ({ ...prev, date: e.target.value }))}
          className="w-full md:w-auto rounded-md border border-input bg-transparent px-2 md:px-3 py-1 text-sm"
        />
      </td>
        ...
        <td key={index} className="hidden md:table-cell p-2 md:p-4 align-middle">
          <div className="flex flex-col gap-2">
            <input
              ...
              value={size.pisces}
              onChange={(e) => handleSizeChange(index, 'pisces', e.target.value)}
              className="w-full rounded-md border border-input bg-transparent px-2 md:px-3 py-1 text-sm"
              placeholder="Pieces"
            />
            <div className="text-sm text-gray-500">Total: {size.total}</div>
          </div>
      ...
      <td className="p-2 md:p-4 align-middle">
        <div className="flex justify-center space-x-2">
          <button
            ...
            className="px-2 md:px-4 py-1 md:py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm"
          >
            Save
          </button>
          <button
            onClick={onCancel}
            className="px-2 md:px-4 py-1 md:py-2 bg-gray-200 rounded hover:bg-gray-300 text-sm"
          >
            Cancel
          </button>
        </div>
...
// Rest of the code remains same until the return statement of ClientData component
// // EditUserModal Component
const EditUserModal = ({ user, onSave, onCancel }) => {
  const [editedUser, setEditedUser] = useState({
    userId: user.userId,
  ...
  <Navbar>
    <div className="container mx-auto py-4 md:py-8 px-2 md:px-4 pt-20">
      <div className="space-y-4 md:space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">ખાતાવહી</h2>
        </div>
        ...
        {error && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-3 md:p-4 text-sm text-red-800">
            <div className="flex items-center space-x-2">
              <span className="font-medium">Error!</span>
              <span>{error}</span>
              <button onClick={() => setError(null)} className="ml-auto">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/50">
                <th className="h-8 md:h-10 px-2 md:px-4 text-left align-middle font-medium">ID</th>
                <th className="h-8 md:h-10 px-2 md:px-4 text-left align-middle font-medium">નામ</th>
                <th className="hidden md:table-cell h-8 md:h-10 px-2 md:px-4 text-left align-middle font-medium">સાઇડ</th>
                <th className="hidden md:table-cell h-8 md:h-10 px-2 md:px-4 text-left align-middle font-medium">મોબાઇલ</th>
                <th className="h-8 md:h-10 px-2 md:px-4 text-left align-middle font-medium">Actions</th>
              </tr>
            </thead>
            <tbody>
              {userData.map((user) => (
                <React.Fragment key={user.userId}>
                  <tr 
                    ...
                    className={`border-b transition-colors hover:bg-muted/50 cursor-pointer
                      ${selectedUser?.userId === user.userId ? 'bg-muted' : ''}`}
                  >
                    <td className="p-2 md:p-4 align-middle">{user.userId}</td>
                    <td className="p-2 md:p-4 align-middle font-medium">{user.name}</td>
                    <td className="hidden md:table-cell p-2 md:p-4 align-middle">{user.site || 'N/A'}</td>
                    <td className="hidden md:table-cell p-2 md:p-4 align-middle">{user.phone || 'N/A'}</td>
                    <td className="p-2 md:p-4 align-middle" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center space-x-2">
                        <button
                          onClick={() => setEditingUser(user)}
                          className="p-1 md:p-2 rounded-md hover:bg-muted"
                        >
                          <Edit className="h-4 w-4" />
                        </button>
                        <button
                  ...
                  {selectedUser?.userId === user.userId && (
                    <tr>
                      <td colSpan="5" className="p-0">
                        <div className="rounded-lg border bg-card m-2 md:m-4">
                          <div className="p-3 md:p-6">
                            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 mb-4 md:mb-6">
                              <h3 className="text-xl md:text-2xl font-semibold text-blue-700">{selectedUser.name}</h3>
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full">
                                <thead>
                                  <tr>
                                    <td className="border-b p-2 md:p-4">
                                      <div className="text-center font-semibold">Stock Status</div>
                                    </td>
                                    <td className="border-b p-2 md:p-4">
                                ...
                                <tbody>
                                      ...
                                      <div className="space-y-4">
                                        {returnItems.map((item) => (
                                          <div 
                                            ...
                                            className={`p-4 rounded-lg ${getRowBackgroundColor(item.receiptNumber)}`}
                                          >
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mb-2">
                                              {item.sizes?.map((size) => (
                                                <div key={size.size}>
                                                  <span className="font-medium">{size.size}:</span> {size.total || '0'}
                                                </div>
                                              ))}
                                            </div>
                                            <div className="flex justify-between items-center">
                                              <span className="text-sm text-gray-600">{item.notes || 'No notes'}</span>
                                  ...
                                  {/* Desktop view for return items */}
                                  <tr className="hidden md:table-row border-b bg-muted/50">
                                    <th className="h-10 px-4 text-left align-middle font-medium">Receipt</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium">Date</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium">Total</th>
                                    {returnItems[0]?.sizes?.map((size) => (
                                      <th key={size.size} className="h-10 px-4 text-left align-middle font-medium">{size.size}</th>
                                    ))}
                                    <th className="h-10 px-4 text-left align-middle font-medium">Notes</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium">Actions</th>
                                  </tr>
                                  {returnItems.map((item) => (
                                    <tr key={item.receiptNumber} 
                                        className={`hidden md:table-row border-b ${getRowBackgroundColor(item.receiptNumber)}`}>
                                      {editingReceipt?.receiptNumber === item.receiptNumber ? (
                                        <EditReceiptRow
                                          receipt={item}
                                          onSave={handleUpdateReceipt}
                                        ...
                                        <>
                                          ...
                                          <td className="p-4 align-middle">{item.total}</td>
                                          {item.sizes?.map((size) => (
                                            <td key={size.size} className="p-4 align-middle text-center">
                                              {size.total || '0'}
                                            </td>
                                          ))}
                                          <td className="p-4 align-middle">{item.notes || 'N/A'}</td>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
            <div className="rounded-lg bg-white p-4 md:p-6 shadow-lg max-w-md w-full">
              <h3 className="text-lg font-semibold mb-4">Confirm Delete</h3>
              <p className="mb-2">Are you sure you want to delete user {userToDelete?.name}?</p>
              <p className="text-red-600 text-sm mb-4 md:mb-6">
                This will also delete all associated receipts.
              </p>
              <div className="flex justify-end space-x-4">
                <button
